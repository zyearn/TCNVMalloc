...
However, a strict policy which return the exact block with least allocation times may take plenty of latencies.
...
Traditional method uses FIFO allocation policy(such as queue) or LRU\cite{zhou2009durable}\cite{rodriguez2015write}.
...
Explicit wear-leveling policy should be applied in two different places in wamalloc:
1) when a thread freed a memory block, this block should be added to the free list of its belonging chunk.
...
2) It is a common case that global heap need to reap the memory chunk returned from local heap, hence there is also a free list in global heap maintaining all usable chunks. When a new thread request a new memory chunk as its in-use chunk, a 

% wear policy
\textbf{wear-aware policy}. How to allocate these free blocks is the key to our wear-aware strategy. 
We adopt different wear-leveling policy in different system level, the reason is discussed in the second section.
In memory block level, all blocks are expected to be allocate evenly.
Thus it is obviously that wamalloc should allocate memory chunk starting from the free pointer first
because these memory blocks are never been allocated since this chunk is assigned to its local heap.
