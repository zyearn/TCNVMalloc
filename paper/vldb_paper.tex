...
% wear policy
\textbf{wear-leveling policy in block level}. 
...
\textbf{wear-leveling policy in chunk level}. 
...
We use several optimizations let the operations of priority queue not to be the bottleneck in wamalloc:
1) 4heap
2) asynchronously

Thus we use our optimized priority queue as the wear-leveling policy used in free chunk pool.

There are two cases when a thead needs to give memory chunk back to the global heap: 
1) when a thread is terminated
2) we define a threshhold value S. Everytime the allocation time of the whole memory chunk exceed S,
the state of chunk is transfered to not-available, waiting to be moved to global heap's priority queue.
