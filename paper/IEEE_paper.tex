...
and the total memory consumption of Wamalloc outperforms that of NVMalloc about 60\% and 10\% under uniform and random workloads.
...
a new memory allocator must be designed for applications running on NVM.
...
when compared with the state-of-the-art allocators.
Specificly, this paper introduces three main techniques of NVM allocator.
First, we propose a novel hybrid(fine-grained and coarse-grained) wear-leveling policy proven to be more accurate;
...
Besides PCM, the other two alternative technologies(memristor and spin transfer torque) are also competing to be candidates for DRAM.
...
a combination of DRAM and NVM should be used in computer systems, taking their respective advantages.
...
Wamalloc uses two sets of size class: small-span(16B, 24B, 32B,..., 256B), large-span(384B, 512B,..., 32768B, 49152B, 65536B). 
...
waiting chunks of the same size class are candidates waiting to be chosen as a new in-use chunk.
...
Thus there exists two different on how to increase the memory space. 
...
In Wamalloc, we use the latter solution for two reasons:
\begin{enumerate}
...
In return we can decrease the potential lock contention by acquiring the lock as less as possible.
    \item Almost all of the modern CPU support page table mechanism,
...
% hybrid method, 异步四叉堆
The best wear-leveling policy should return the least allocated chunks so far. 
...
in which it manages the block information inside a chunk and some chunk-related data,
...
\If {$\text{isSmallClass}(sc)$}
    ...
    \If {$\text{IsFullChunk(}\textit{ch}\text{)}$}
...
and it is more complicated than just allocating the memory in a multithreading environment
...
and an elaborate hybrid wear-leveling policy to improve the lifetime of NVM.
...
We thank anonymous reviewers for their insightful and helpful comments.
We also thank Yanyan Shen for her constructive suggestions and comments on this paper.
